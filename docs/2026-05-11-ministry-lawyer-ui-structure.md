...
## Status — 2026-05-11 (epic [#714](https://github.com/henrikaavik/Seadusloome/issues/714) — SHIPPED)

This document is the design rationale; only a subset of what it describes was implemented in the first round, and it shipped. Read the rest of the doc as long-term direction, **not** as the current product.

**Shipped (10 PRs, all merged to `main`):**

- **Phase A — framing:** nav reframe *in place* — `Uurija`→`Õiguskaart`, `Vestlus`→`Nõustaja`, new `Analüüsikeskus` entry; URLs unchanged (`/explorer`, `/chat`, `/drafts`, `/drafter` stay; only `/analyysikeskus` is new) (#726). Estonian diacritics sweep (#727). `Töölaud` rewritten as an operational work queue — next actions / high-risk findings / stale analyses / ontology syncs / recent exports / drafts with open annotations / bookmarks (#730). `Õiguskaart` control bar relabelled to legal-work language (`Lähtesta paigutus` / `Näita/peida seosenimed` / `Rühmita liigi järgi` behind a `Vaate seaded` disclosure; `Ajaline vaade`) (#728). `?focus=<uri>` wiring — report links open the graph focused on the entity, with a "← Tagasi aruandesse" link (#729).
- **Phase B — Analüüsikeskus MVP:** `/analyysikeskus` directory page + the `Sisend → Ulatus → Tulemused → Tõendid → Soovitatud tegevused` result shell (#731). **`Normi mõjuahel`** (#732) — provision/draft/CELEX/case-number/NL `sisend` → resolved entity → impact-chain findings (`Peamised mõjud` / `Kõrge riskiga seosed` / `Seotud eelnõud` / `Riigikohtu praktika` / `EL seosed` + risk band + recommendation) reusing the existing impact engine via an ephemeral synthetic Jena graph; a draft UUID reuses the stored `impact_reports` row. **`EL ülevõtt ja harmoneerimine`** (#733) — CELEX/title `sisend` → an **act/provision-level transposition table** (`EL õigusakt | Eesti õigusakt(id) | Eesti säte(d) | Staatus kaetud/osaline/puudub/ebaselge | Soovitatud tegevus`). Cross-links — `Ava analüüsikeskuses` from draft detail + impact report; `Küsi nõustajalt selle leiu kohta` from result rows via a server-side single-use `pending_chat_seed` token (the finding never travels through a URL) (#734).
- `Soovitatud tegevused` on every workflow result is a **static** action set (`Ava õiguskaardil` / `Küsi nõustajalt` / `Tagasi`, + `Lisa märkus` on draft-backed analyses) — **not** LLM-generated. No SPARQL/RDF/named-graph/embedding language anywhere in the user-facing copy.

**Deferred to a follow-up epic (TBD):**

- **Section-7 workflows 3–8** (`Pädevused ja järelevalve`, `Sanktsioonide võrdlus`, `Kohustused/keelud/halduskoormus`, `KOV võrdlus`, `Kriisikaart`, `Avaliku teenuse tervikvaade`) — *including `Pädevused`, which this doc's Phase B list names*. Their **ontology data largely already exists** in the `estonian-legal-ontology` source repo (`Sanction` nodes with structured penalty values, institution-authority nodes, KOV coverage, …, well ahead of the `estonian-legal-ontology-plan.md` snapshot in this repo) — the gap is **app-side SPARQL + UI integration**, not data. The follow-up epic should re-audit the ontology repo, not the plan doc. No placeholder cards for these in the meantime.
- The **article/obligation-level transposition matrix** described under "Workflow 2" / "Concrete Page Recommendations" — the ontology has no EU Article/Obligation entity model; that's its own ontology-enrichment ticket. The shipped `EL ülevõtt` is act/provision-level.
- `Ülevaatus` review queue (Phase E). The **LLM advice / suggested-fix engine** (Phase D). The `Koostaja` `Lahendusvariandid` step. A light reading theme. URL relocation. The `app/drafter/prompts.py` VTK-template section-title diacritics (they double as `VTK_SECTION_PROMPTS` lookup keys — needs a resilient-lookup change first).
- **Refactor follow-up:** `app/analyysikeskus/routes.py` is ~2.2k lines — split it into a `routes/` package (`_directory.py` / `_normi_mojuahel.py` / `_el_ulevott.py` / shared), like `app/docs/routes/`.
