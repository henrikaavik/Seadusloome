...
**Status:** SHIPPED (epic #762, 2026-05-12) — workstreams A–G all merged into `main`; see the Status section at the foot of this doc for the per-workstream summary. Follows on from `docs/2026-05-12-ui-plan-explorer-home.html` (phases 1–3 of that report shipped as #746/#747: authenticated `/` → `/dashboard`; Õiguskaart now wears the standard `PageShell` chrome with an in-page toolbar; `?focus=<uri>` deep-links work from the impact report and Analüüsikeskus). This doc covered what was left — the report's phases 4–5, its "Make Õiguskaart More Useful" items, and the additional asks from the 2026-05-12 review.
...
- **2026-05-12 (epic #762 shipped)** — all workstreams A–G merged into `main`:
  - **A (#754)** — focus-first entry: a cold `/explorer` (no `?focus=` / `?draft=` / `?search=` / `?vaade=koik`) renders the contextual start panel (search · your bookmarks · recent high-risk findings · recent drafts · `Normi mõjuahel` shortcut · "Sirvi liikide kaupa"); the 90k graph loads only after a choice / `?vaade=koik`. Org-scoped DB queries in `app/explorer/start_panel.py`.
  - **B (#755)** — `?draft=<id>` → the draft's impact subgraph (affected / conflict / gap provisions + inter-relations), reusing the stored `impact_reports` row; "← Tagasi eelnõu juurde" back to the draft; a graceful "run the analysis" fallback when there's no report yet. Data endpoint `/explorer/draft-subgraph/{id}`.
  - **C (#756)** — five legal-view preset chips in the toolbar (`Kehtiv õigus` · `Eelnõu mõjud` · `EL seosed` · `Kohtupraktika` · `Ajalugu`); raw graph knobs stay under `Vaate seaded ▾`; presets are URL-addressable (`?vaade=<slug>`) and mirror back into the URL on click.
  - **D (#757)** — the node detail panel is an evidence card: Allikas · Kuupäev / versioon · Seose liik (legal language) · Miks see oluline on (deterministic one-liner) · Tegevused (`Küsi nõustajalt` → `/chat/seed` · `Ava analüüsikeskuses` → `/analyysikeskus/normi-mojuahel?sisend=` · `Lisa märkus` · `Lisa järjehoidja`).
  - **E (#758)** — spatial-map polish: mini-map (overview panel + draggable viewport rect, bottom-right), zoom-to-region (double-click a node → fit its cluster; shift-drag → fit a rubber-banded rectangle), a "you are here" amber ring + 📍 pin + "Te olete siin" badge on the focused node, and a stable layout (FNV-1a-seeded node positions + a fixed d3-force PRNG) so re-opening the same query reproduces the same picture.
  - **F (#759)** — deep-links *in*: Koostaja ontology-research cards ("Ava õiguskaardil →"), assistant-message provision citations ("vaata kaardil"), the draft detail page ("Vaata mõjukaarti"), and an `explorer_draft_url(draft_id)` helper next to `explorer_focus_url`.
  - **G (#760, this PR)** — responsive + accessibility QA pass over the combined result:
    - *Keyboard reachability* — a visible `:focus-visible` ring (design-system `--color-focus-ring`) on every bespoke control: the toolbar buttons, the preset chips, the search box, the `Vaate seaded` disclosure summary, the back links, the timeline slider, the detail-panel action buttons, and every start-panel control. The breadcrumb crumbs and the detail-panel neighbour-list rows became keyboard-operable (`role="button"` + `tabindex="0"` + Enter/Space) instead of click-only. The closed detail panel is `visibility: hidden` so its buttons aren't a "ghost focus" trap; the mini-map svg is `focusable="false"` so it can't trap keyboard focus.
    - *ARIA* — `role="region"` + `aria-label` on the start panel and on the detail panel (with `aria-hidden` toggled by `explorer.js` on open/close, and focus moved to the close button on open / restored to the opener on close); `role="search"` on the toolbar search box; the preset chip group already had `role="group"` + `aria-pressed` (verified); `role="img"` + `aria-label` on the mini-map wrapper with the inner svg `aria-hidden`; the breadcrumb is a labelled `<nav>` landmark; the chrome behind the contextual start panel is `inert` while it's up so keyboard/AT focus stays in the panel. (Toasts were already in PageShell's `aria-live="polite"` container.)
    - *Responsive* — extended the `@media (max-width:768px)` block to cover the preset chips (full-width wrapping row that can scroll-x), the evidence card (long source labels / dates / metadata wrap; the panel goes full-content-width), and the mini-map (shrunk to 132×96), plus a new `@media (max-width:400px)` block (hide the page-identity label, the cold-open tip, and the mini-map; tighten the legend/instructions/timeline) — at 360px and 768px the start panel, toolbar, graph canvas, mini-map, and detail panel are all usable with no overlap and no horizontal scroll.
    - *Reduced motion* — `@media (prefers-reduced-motion: reduce)` neutralises the #758 "you are here" pulsing ring (now a static amber ring), the loading spinner, the toast slide-in/out (kept the auto-fade so toasts still clear), and the detail-panel slide (the visibility toggle is preserved so tab order stays correct).
    - *Screenshots* — **skipped**: this repo has no headless-browser harness, so the desktop + mobile screenshot DoD item could not be completed without faking it; the behavioural changes are covered by `tests/test_explorer_pages.py` (ARIA roles/labels in the markup, the `:focus-visible` / `prefers-reduced-motion` / `@media` rules via string match, the JS keyboard/focus helpers).
