...
# Step-page renderers extracted into a sibling module (#... — routes.py
# size refactor). All 13 names are re-bound into this module's namespace
# so existing patch paths (``patch("app.drafter.routes._step_N_page")``,
# ``patch("app.drafter.routes._find_latest_job")``, etc.) keep working.
# The ``noqa: F401`` marker keeps the rebinds that aren't *called* from
# routes.py itself (e.g. ``_step_1_page``, ``_step_waiting_page``) from
# being stripped as "unused" by ruff.
from app.drafter._step_renderers import (  # noqa: E402, F401
    _find_latest_job,
    _render_clause_card,
